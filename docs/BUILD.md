...
## Toolchain used for 1.0.0 and 1.1.0-rc5
...
python scripts/build.py `
  ...
  --output work/Oniimai-Kanade-API102-1.1.0-rc5.apk
```
...
python scripts/test_phone_nfc_handoff.py --build-dir work/nfc-handoff-tests
python scripts/audit_release.py --apk work/Oniimai-Kanade-API102-1.1.0-rc5.apk
```

`javac` and `java` must be on PATH. The NDK option also runs C++ state tests; without it only Java checks run. The USB transport and phone NFC handoff tests run production transport classes against isolated Android fakes, not physical hardware. Unix paths can be supplied to the builder; the documented release environment is Windows. Do not interpret host tests as physical USB, NFC or display certification.
