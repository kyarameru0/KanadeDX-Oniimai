...
The default output in the build guide is `work/Oniimai-Kanade-API102-1.1.0-rc3.apk`. Select your test device explicitly if more than one is connected:

```powershell
adb devices
$recipeDevice = 'YOUR_DEVICE_SERIAL'
adb -s $recipeDevice install --no-incremental -r work/Oniimai-Kanade-API102-1.1.0-rc3.apk
